...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
    if (dalvikJavaVMPtr == NULL) {
        ...
        class_MainActivity = (*env)->NewGlobalRef(env,(*env)->FindClass(env, "zx/offical/quattro/MainActivity"));
...
JNIEXPORT void JNICALL Java_zx_offical_quattro_AWTInputBridge_nativeSendData(JNIEnv* env, jclass clazz, jint type, jint i1, jint i2, jint i3, jint i4) {
...
JNIEXPORT jboolean JNICALL Java_zx_offical_quattro_utils_JREUtils_renderAWTScreenFrame(JNIEnv* env, jclass clazz, jobject targetBuffer) {
...
JNIEXPORT void JNICALL Java_zx_offical_quattro_AWTInputBridge_nativeClipboardReceived(JNIEnv *env, jclass clazz, jstring clipboardData, jstring clipboardDataMime) {
...
Java_zx_offical_quattro_AWTInputBridge_nativeMoveWindow(JNIEnv *env, jclass clazz, jint xoff, jint yoff) {
    if (runtimeJNIEnvPtr_INPUT == NULL) {
        if (runtimeJavaVMPtr == NULL) {
            return;
        } else {
